...
Set **API base URL**, **project ID** (UUID, same as the key’s project), and **project API key** in the UI (or via `VITE_PLANVAULT_*` in `.env`).
...
After **`POST .../projects/{projectId}/sessions/{id}/messages`** (**HTTP 202**), the UI shows **`messageId`** from the JSON body and polls **`GET .../messages/{messageId}/status`** every 2s until `completed` or `failed` (in addition to the existing SSE stream for tool/plan events).
