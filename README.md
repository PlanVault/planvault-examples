...
- **Send prompts** — use `POST /api/v1/sessions/{id}/messages` with `{"message":"...","autoExecute":true|false}` (this repo’s React sample and `scripts/bash-e2e` already do). The response includes **`messageId`** while planning runs asynchronously; poll **`GET /api/v1/sessions/{id}/messages/{messageId}/status`** for coarse phases (`planning`, `executing`, `completed`, …) or rely on SSE **`GET .../chat`** / **`GET .../history`** as before. Legacy `POST .../prompt` is deprecated.
