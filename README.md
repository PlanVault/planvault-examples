...
- **Create session** (`POST /api/v1/sessions`): body still requires `externalUserId` and **`contextVars`** (JSON object; use `{}` if none). Optional: `model`, `secrets`, `prompt`, `stream` — see **[API documentation](https://planvault.ai/api-docs)** (includes embedded Swagger).
